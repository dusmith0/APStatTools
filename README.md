...
Tail can be any of Input String being any of "left", "right", "inner", "outer" or "two", "left_not_equal", "right_not_equal". Note: "left_not_equal" and "left_not_equal" are intended for the "binomial" only. Also "out" and "two" will produce the same output.
...
get_data("potter.csv")
```
You can print the data by changing get_pdf = FALSE. Note thought that the function will then require you to input a desired save path.
```{r, eval = FALSE, echo = TRUE}
get_data("potter.csv", get_pdf = FALSE)
```
...
size = amount of people included in a room
plot = FALSE to suppress the graphics. 
