test_that("test preforms the one sample t-test correclty", {
  found <- test(test = "t_test.one", null = 10, x_bar = 8, sigma = 3, n = 25, tail = "left", graph = FALSE)
  null <- 10
  x_bar <- 8
  sigma <- 3
  n <- 25
  ...
  expect_equal(found$standard_error, sigma/sqrt(n))
  ...
  found <- test(test = "t_test.one", null = 10, x_bar = 8, sigma = 3, n = 25, tail = "right", graph = FALSE)
  expect_equal(found$p_value, pt((8 - 10)/(3/sqrt(25)),25-1, lower.tail = FALSE))

  #testing for two tails
  found <- test(test = "t_test.one", null = 10, x_bar = 8, sigma = 3, n = 25, tail = "two", graph = FALSE)
...
test_that("test preforms the two sample t-test correclty", {
  found <- test(test = "t_test.two", null = 0, x_bar = c(12,9), sigma = c(2,3), n = c(25,30), tail = "left", graph = FALSE)
  null <- 0
  x_bar <- c(12,9)
  sigma <- c(2,3)
  n <- c(25,30)
  ...
  expect_equal(found$standard_error, c(sigma[1]/sqrt(n[1]), sigma[2]/sqrt(n[2])))
  expect_equal(round(found$df,3), round(c(50.742,50.742),3))
  expect_equal(found$test_statistic[1], (12 - 9)/(sqrt( sigma[1] ^ 2 / n[1] + sigma[2] ^ 2 / n[2])))
  expect_equal(found$p_value[1], pt((12 - 9)/(sqrt( sigma[1] ^ 2 / n[1] + sigma[2] ^ 2 / n[2])),50.742))

  #Testing for right tails
  found <- test(test = "t_test.two", null = 0, x_bar = c(12,9), sigma = c(2,3), n = c(25,30), tail = "right", graph = FALSE)
  expect_equal(found$p_value[1], pt((12 - 9)/(sqrt( sigma[1] ^ 2 / n[1] + sigma[2] ^ 2 / n[2])),50.742007,lower.tail = FALSE))

  #testing for two tails
  found <- test(test = "t_test.two", null = 0, x_bar = c(12,9), sigma = c(2,3), n = c(25,30), tail = "two", graph = FALSE)
  expect_equal(found$p_value[1], pt((12 - 9)/(sqrt( sigma[1] ^ 2 / n[1] + sigma[2] ^ 2 / n[2])),50.742007,lower.tail = TRUE) * 2)
...
test_that("test preforms confidence intervals correctly",{
  found <- test(test = "t_conf.one", x_bar = 12, sigma = 3, level = c(.95), n = 20)

  expect_equal(found$lower_bound, (12 + -1 * qt(.975,19) * 3 / sqrt(20)))
  expect_equal(found$upper_bound, (12 + 1 * qt(.975,19) * 3 / sqrt(20)))

  found <- test(test = "t_conf.two", x_bar = c(12,10), sigma = c(3,2), level = .95, n = c(20,30))
  df <- ((3 ^ 2 / 20 + 2 ^ 2 / 30) ^ 2) / sum((1 / (c(20,30) - 1)) * (c(3,2) ^ 2 / c(20,30)) ^ 2)
  expect_equal(found$lower_bound, ((12 - 10) + -1 * qt(.975,df) * sqrt(9 / 20 + 4 / 30)))
  expect_equal(found$upper_bound, ((12 - 10) + 1 * qt(.975,df) * sqrt(9 / 20 + 4 / 30)))

  found <- test(test = "z_conf.one", p_hat = .4, level = .90, n = 20)
  expect_equal(found$lower_bound, (.4 + -1 * qnorm(.95) * sqrt(.4 * .6 / 20)))
  expect_equal(found$upper_bound, (.4 + 1 * qnorm(.95) * sqrt(.4 * .6 / 20)))

  found <- test(test = "z_conf.two", p_hat = c(.4,.6), level = .99, n = c(20,30))
  expect_equal(found$lower_bound, ((.4 - .6) + -1 * qnorm(.995) * sqrt((.4 * .6 / 20) + (.6 * .4 / 30))))
  expect_equal(found$upper_bound, ((.4 - .6) + 1 * qnorm(.995) * sqrt((.4 * .6 / 20) + (.6 * .4 / 30))))
})
